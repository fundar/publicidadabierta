...
class Manager extends CI_Model {
				...
				if(is_array($value)) { // if folders			
					...
					{							
						...
						switch($file_ext)
						{
							...
							case 'jpeg':
							case 'ico':
								$img_url = $anchor_url = 'assets/'.$params->media_path.'/'.$path.$value;													
								...
								if (($info[0] > $this->img_width) || ($info[1] > $this->img_width))
								{
									...
									$url = 'assets/'.$params->media_path.'/'.$path.'thumb/'.$value;									
							...
							default: 
								...
								$data['docs'][$file] = array(
										'name'	      =>	$value,										
										'path'		  =>	$path.$value,									
										'doc_url'	  =>	'assets/'.$params->media_path.'/'.$path.$value,
	...
	{
		$params = json_decode(read_file(realpath(FCPATH . 'assets/params.json')));		
		$default_params = json_decode(read_file(realpath(FCPATH . 'assets/default.json')));	
